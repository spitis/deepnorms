...
# ==============================================================================
#
# EDITED WITH CUSTOM PAIRWISE DISTANCE FNS
...
from metrics import euclidean_metric, mahalanobis_metric, widenorm_metric, deepnorm_metric, mlp_nonmetric,\
...
def pairwise_distance(feature, squared=False):
  ...
  if squared:
    ...
    pairwise_distances = math_ops.sqrt(pairwise_distances_squared +
                                       math_ops.to_float(error_mask) * 1e-16)
...
def make_pairwise_distance_mixture_mahalanobis(n,
                                               m=64,
                                               k=1,
                                               use_euclidean_component=True,
                                               variable_collection=None):
  Ws = []
  for i in range(k):
    Ws.append(
        tf.get_variable('mm_W_{}'.format(i), shape=(n, m), dtype=tf.float32))
    if variable_collection is not None:
      tf.add_to_collection(variable_collection, Ws[-1])

  def pd(a, b=None, squared=False):
    ds = []
    if use_euclidean_component:
      ds = [pairwise_distance_euclidean(a, b, squared=False)]
    for W in Ws:
      wa = tf.matmul(a, W)
      if b is not None:
        wb = tf.matmul(b, W)
        ...
        wb = None
      ds.append(pairwise_distance_euclidean(wa, wb, squared=False))

    pairwise_distances = tf.reduce_mean(ds, axis=0)
    if squared:
...
def make_pairwise_distance_metric(m):

  def pairwise_distance_dn(a, b):
    # a.shape = N x D
    # b.shape = M x D
    N, D = tf.shape(a)[0], a.get_shape()[1]
    M = tf.shape(b)[0]

    a = tf.tile(tf.expand_dims(a, axis=1), (1, M, 1))  # [N, M, D]
    b = tf.tile(tf.expand_dims(b, axis=0), (N, 1, 1))  # [N, M, D]

    hx = tf.reshape(b, [N * M, D])
    hy = tf.reshape(a, [N * M, D])

    metric = m.split('_')
    if metric[0] == 'euclidean':
      p = euclidean_metric(hx, hy)
    
    elif metric[0] == 'mahalanobis':
      size = int(metric[1])
      p = mahalanobis_metric(hx, hy, size)
    
    elif metric[0] == 'widenorm':
      num_components, component_size, concave_activation_size, mode, symmetric = metric[1:]
      p = widenorm_metric(hx, hy, int(num_components), int(component_size), 
      ...
      p = deepnorm_metric(hx, hy, layers, activation, int(concave_activation_size),
                             mode, bool(symmetric))
      ...
      layer_size, num_layers = layers.split('x')
      layers = [int(layer_size)]*int(num_layers)
      p = mlp_nonmetric(hx, hy, layers, mode)
    
    else:
      raise ValueError('Unsupported metric')

    return tf.reshape(p, [N, M])

  return pairwise_distance_dn
...
def contrastive_loss(labels, embeddings_anchor, embeddings_positive,
  ...
  return math_ops.reduce_mean(
      ...
      (1. - math_ops.to_float(labels)) * math_ops.square(
          math_ops.maximum(margin - distances, 0.)),
...
def triplet_semihard_loss(labels,
                          embeddings,
                          margin=1.0,
                          pairwise_distance_fn=pairwise_distance):
  ...
  pdist_matrix = pairwise_distance_fn(embeddings, squared=True)
  ...
  for i in range(num_pairs):
    for j in range(num_pairs):
      ...
      one_hot_matrix = array_ops.pad(
          sparse_dot_product, [[i, num_pairs - i - 1], [j, num_pairs - j - 1]],
          'CONSTANT')
...
def npairs_loss_multilabel(sparse_labels,
  ...
  if False in [
    ...
    raise TypeError('sparse_labels must be a list of SparseTensors, but got %s'
                    % str(sparse_labels))
...
def lifted_struct_loss(labels,
  ...
  # Build pairwise squared distance matrix.
  pairwise_distances = pairwise_distance_fn(embeddings)
...
def _find_loss_augmented_facility_idx(pairwise_distances, labels, chosen_ids,
  ...
  argmax_index = math_ops.to_int32(math_ops.argmax(candidate_scores, axis=0))
...
def update_all_medoids(pairwise_distances, predictions, labels, chosen_ids,
  ...
  def func_body_augmented_pam(iteration, chosen_ids):
    ...
    chosen_ids = update_medoid_per_cluster(
        pairwise_distances, pairwise_distances_subset, labels, chosen_ids,
